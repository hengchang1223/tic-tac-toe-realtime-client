...
class ShowUsers extends Component {
    ...
    componentDidMount() {
        ...
        this.props.socket.on('newOpponentAdded', data => {
            // console.log(data);
            if (data.gameMode === this.state.gameMode) {
                this.setState({
            ...
            <Fragment>
                ...
                <ListGroup onSelect={this.selectOpponent}>
                    {this.state.opponents.map(function (opponent, index) {
                        return <ListGroup.Item action={true} className="opponent-item" key={index} eventKey={index} >{opponent.username} | Played : {opponent.played}  | Won : {opponent.won}  | Draw : {opponent.draw}</ListGroup.Item>;
                    })}
                </ListGroup></Fragment>
        );
    }
